...
public sealed class SalesExcelExportRenderer : ISalesExcelExportRenderer
{
    public Task<SalesExportResult> RenderAsync(SalesExportDatasetDto dataset, CancellationToken cancellationToken)
    {
        ...
        var isLineItem = string.Equals(dataset.Metadata.ExportLevel, SalesExportLevel.LineItems, StringComparison.OrdinalIgnoreCase);

        if (isLineItem)
        {
            return RenderLineItemsAsync(workbook, dataset);
        }

        return RenderSummaryAsync(workbook, dataset);
    }

    private static Task<SalesExportResult> RenderSummaryAsync(XLWorkbook workbook, SalesExportDatasetDto dataset)
    {
    ...
    private static Task<SalesExportResult> RenderLineItemsAsync(XLWorkbook workbook, SalesExportDatasetDto dataset)
    {
    ...
    private static LineItemColumnPlan RenderLineItemHeaders(IXLWorksheet worksheet, int startRow)
    {
        var col = 1;
    ...
    private static void RenderLineItemRow(
    ...
    private static void RenderLineItemTotalsRow(
        IXLWorksheet worksheet,
    ...
    private static void ApplyLineItemPolishing(IXLWorksheet worksheet, int totalColumnsEnd, int headerRow)
    {
