...
- Domain has no `<PackageReference>` or `<ProjectReference>` at all — verified in `src/backend/InventoryAI.Domain/InventoryAI.Domain.csproj`
- Application references only Domain: `src/backend/InventoryAI.Application/InventoryAI.Application.csproj`
- Infrastructure references Domain + Application: `src/backend/InventoryAI.Infrastructure/InventoryAI.Infrastructure.csproj`
- API references Application + Infrastructure: `src/backend/InventoryAI.Api/InventoryAI.Api.csproj`
...
**Where it appears:**
- `src/backend/InventoryAI.Application/DependencyInjection.cs:8` — `AddApplication()`
- `src/backend/InventoryAI.Infrastructure/DependencyInjection.cs:13` — `AddInfrastructure()`
- `src/backend/InventoryAI.Api/Program.cs:11-12` — call site
...
**Where it appears:**
- Contract: `src/backend/InventoryAI.Domain/Interfaces/Repositories/IRepository.cs:6-14`
- Implementation base: `src/backend/InventoryAI.Infrastructure/Repositories/RepositoryBase.cs:9-28`
...
**Where it appears:**
- Interface: `src/backend/InventoryAI.Domain/Interfaces/IUnitOfWork.cs:3-6`
- Implementation: `src/backend/InventoryAI.Infrastructure/Data/UnitOfWork.cs:5-8`
- DI registration: `src/backend/InventoryAI.Infrastructure/DependencyInjection.cs:27`
...
**Where it appears:**
- Error definitions: `src/backend/InventoryAI.Application/Common/Errors/Errors.cs:8-20` — `partial class Errors` with nested static classes per aggregate
- HTTP mapping: `src/backend/InventoryAI.Api/Extensions/ErrorOrExtensions.cs:8-40` — `ToActionResult<T>()` and `ToProblemResult()`
- Validation integration: `src/backend/InventoryAI.Application/Common/Behaviours/ValidationBehaviour.cs:28-29` — FluentValidation failures become `Error.Validation`
...
**Where it appears:**
- Options class: `src/backend/InventoryAI.Infrastructure/Options/DatabaseOptions.cs:5-25` — `[Required]` and `[Range]` annotations enable `ValidateDataAnnotations()`
- Registration: `src/backend/InventoryAI.Infrastructure/DependencyInjection.cs:15-18` — `.ValidateDataAnnotations().ValidateOnStart()` causes startup failure on misconfiguration
- Config section key: `DatabaseOptions.SectionName` constant at `src/backend/InventoryAI.Infrastructure/Options/DatabaseOptions.cs:7`
...
**Where it appears:**
- Event interface: `src/backend/InventoryAI.Domain/Common/IDomainEvent.cs:3-7`
- Collection on entity: `src/backend/InventoryAI.Domain/Common/BaseEntity.cs:9-13` — private `_domainEvents` list, public `AddDomainEvent` / `ClearDomainEvents`
...
**Where it appears:**
- Base class: `src/backend/InventoryAI.Domain/Common/ValueObject.cs:3-18` — `GetEqualityComponents()` abstract method; `==` / `!=` operators and `IEquatable<ValueObject>` wired up
...
**Where it appears:**
- Discovery call: `src/backend/InventoryAI.Infrastructure/Data/ApplicationDbContext.cs:11` — `ApplyConfigurationsFromAssembly(...)` scans the Infrastructure assembly automatically
- Config classes go in: `src/backend/InventoryAI.Infrastructure/Data/Configurations/` *(empty, to be populated)*
...
**Where it appears:**
- `src/backend/InventoryAI.Infrastructure/Data/ApplicationDbContext.cs:21-29` — iterates `ChangeTracker.Entries<BaseEntity>()` before delegating to base
- Audit properties defined: `src/backend/InventoryAI.Domain/Common/BaseEntity.cs:5-7`
...
**Where it appears:**
- Implementation: `src/backend/InventoryAI.Api/Middleware/ExceptionHandlingMiddleware.cs:6-44`
- Registration (first middleware): `src/backend/InventoryAI.Api/Program.cs:21`
...
**Where it appears:**
- `src/backend/InventoryAI.Api/Program.cs:14-17` — `UseWolverine` with `IncludeAssembly`
- `src/backend/InventoryAI.Application/Common/Behaviours/ValidationBehaviour.cs` — Wolverine middleware (`BeforeAsync`) integrating FluentValidation
