...
| [19](#19-one-resource-group-for-everything) | Resource groups | One for everything | Per-environment groups |
| [20](#20-no-server-side-rendering) | Web rendering | Client-side, static files + proxy | Angular SSR |
| [21](#21-jwt-signing-moves-into-key-vault) | JWT signing | RS256 with a vault-generated key | Shared HMAC secret |
...
## 21. JWT signing moves into Key Vault

*Added 2026-07-26, replacing the shared HMAC secret that §7 assumed.*

**Context.** The original plan generated a random string, stored it as a Key Vault secret, and delivered it to the API, which signed HS256 with it. That works, and it means the signing key exists in plaintext at generation time, on whichever machine generated it, and in the memory of every replica for as long as it runs. Anything that reads it can mint a token for any user in any shop, and nothing about that is detectable.

**What Key Vault can actually generate.** Verified against the live dev vault rather than assumed:

- `oct` → *"Software oct key type is not supported. Use oct-HSM."*
- `oct-HSM` → *"Hardware key operation not allowed on standard vault"* — Managed HSM only, which costs more per hour than this estate costs per month.
- `RSA` with `sign`/`verify` → created, with a rotation policy available.

So a vault-generated signing key requires asymmetric signing. That is not a workaround; it is the better arrangement.

**Decision.** RS256, with an RSA key generated by Key Vault and owned by Terraform. The API signs through the vault and validates with the public half.

**What it buys.**

- The private key has **no export operation**. It is not in state, not in a shell, not on a laptop, not in the container. A compromised API can ask for signatures while its access lasts — revocable and audited — instead of walking away with the ability to mint tokens forever.
- Rotation is a vault policy (`P1Y` expiry, new version 30 days ahead) rather than a runbook step someone remembers. Validation resolves keys by `kid`, so a rotation invalidates nothing already issued.
- Terraform owns the key without owning any secret material, which is what §7 wanted and could not have while the key was symmetric.

**What it costs.**

- One vault call per token **issued** — login and refresh, not per request. Roughly 20–60 ms on those paths, and about $0.03 per 10,000 operations.
- A Key Vault outage stops new logins and refreshes. Tokens already issued keep validating from the cached public keys, so it degrades rather than fails.
- The token is assembled by hand. `JwtSecurityTokenHandler` signs synchronously, and blocking a request thread on a network call is how a thread pool starves.

**Local development is unchanged.** `Jwt:SigningMode` defaults to `Hmac`, so a developer machine and the test suite need no Azure at all. `JwtOptionsValidator` refuses to start with both a secret and Key Vault settings, because that combination almost always means one environment's configuration reached another.

**Verified end to end** against `intelibill-dev-kv`: a registration returned a token with `alg: RS256` and a `kid` matching the applied key version, a 256-byte signature, an authenticated call answering 200, and a tampered token answering 401.

**Revisit when** token issuance rate makes the per-login vault call material — the fix then is a short-lived cached signature path or a Managed HSM, not a return to a shared secret.
