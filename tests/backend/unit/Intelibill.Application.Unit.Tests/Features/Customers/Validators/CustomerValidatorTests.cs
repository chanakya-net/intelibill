...
public class AddCustomerCommandValidatorTests
{
    ...
    [Fact] public void Validate_WhenNameEmpty_ReturnsError() =>
        _v.TestValidate(new AddCustomerCommand(Guid.NewGuid(), "", "+919876543210", null, true))
            .ShouldHaveValidationErrorFor(x => x.Name);

    [Fact] public void Validate_WhenPhoneEmpty_ReturnsError() =>
        _v.TestValidate(new AddCustomerCommand(Guid.NewGuid(), "Name", "", null, true))
            .ShouldHaveValidationErrorFor(x => x.PhoneNumber);

    [Fact] public void Validate_WhenPhoneInvalid_ReturnsError() =>
        _v.TestValidate(new AddCustomerCommand(Guid.NewGuid(), "Name", "abc", null, true))
            .ShouldHaveValidationErrorFor(x => x.PhoneNumber);

    [Fact] public void Validate_WhenAddressTooLong_ReturnsError() =>
        _v.TestValidate(new AddCustomerCommand(Guid.NewGuid(), "Name", "+919876543210", new string('x', 321), true))
            .ShouldHaveValidationErrorFor(x => x.Address);

    [Fact] public void Validate_WhenValid_NoErrors() =>
        _v.TestValidate(new AddCustomerCommand(Guid.NewGuid(), "Name", "+919876543210", null, true))
...
public class EditCustomerCommandValidatorTests
{
    ...
    [Fact] public void Validate_WhenCustomerIdEmpty_ReturnsError() =>
        _v.TestValidate(new EditCustomerCommand(Guid.NewGuid(), Guid.Empty, "Name", "+919876543210", null, true))
            .ShouldHaveValidationErrorFor(x => x.CustomerId);

    [Fact] public void Validate_WhenNameEmpty_ReturnsError() =>
        _v.TestValidate(new EditCustomerCommand(Guid.NewGuid(), Guid.NewGuid(), "", "+919876543210", null, true))
            .ShouldHaveValidationErrorFor(x => x.Name);

    [Fact] public void Validate_WhenPhoneInvalid_ReturnsError() =>
        _v.TestValidate(new EditCustomerCommand(Guid.NewGuid(), Guid.NewGuid(), "Name", "notaphone", null, true))
            .ShouldHaveValidationErrorFor(x => x.PhoneNumber);

    [Fact] public void Validate_WhenValid_NoErrors() =>
        _v.TestValidate(new EditCustomerCommand(Guid.NewGuid(), Guid.NewGuid(), "Name", "+919876543210", null, true))
            .ShouldNotHaveAnyValidationErrors();
}
